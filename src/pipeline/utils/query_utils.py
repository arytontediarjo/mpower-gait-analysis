...
def get_walking_synapse_table(syn,
    ...
    -> Querying process:
        1.) Query data usings synapse tableQuery on non-iOS data (for now),
        then parse it to dataframe
        2.) mpower v1 and elevate MS "device_motion" data will be taken,
        and all json files from mpowerV2 and mpower_passive will be taken
        3.) Use synapse downloadTableColumns to download all
        the designated column files from the table
        4.) Inner join the parsed dataframe from synapse table query dataframe
        with download Table Columns by their file handle ids

    Args:
        syn              : synapseclient object
        table_id    (str): id of table entity
        version     (str): version number;
        (options = ["MPOWER_V1", "MPOWER_V2", "MS_ACTIVE", "PASSIVE"])
        healthcodes (str): list or array of healthcodes
        recordIDs   (str): list or of recordIds

    Return:
        RType: pd.DataFrame
        returns a pandas dataframe of recordIds and their metadata,
        file handle ids and their filepaths
    """
    print("Querying %s Data" % table_version)

    if not retrieveAll:
        if not isinstance(recordIds, type(None)):
            recordId_subset = "({})".format(
                [i for i in recordIds])\
                .replace("[", "")\
                .replace("]", "")
            query = syn.tableQuery(
                "select * from {} WHERE recordId in {}"
                .format(table_id, recordId_subset))
        else:
            healthCode_subset = "({})".format(
                [i for i in healthCodes])\
                .replace("[", "")\
                .replace("]", "")
            query = syn.tableQuery(
                "select * from {} WHERE healthCode in {}"
                .format(table_id, healthCode_subset))
    else:
        query = syn.tableQuery(
            "select * from {} where \
            phoneInfo like '%iPhone%'\
            or phoneInfo like '%iOS%'"
            .format(table_id))
...
def save_data_to_synapse(syn,
    ...
    Args:
        ...
        data_parent_id (list, np.array)  : the parent folder synid
        used_script (list, np.array)     : executed github url
        source_table_id (list, np.array) : list of used data source
        remove (boolean)                 : prompt to remove data
...
def norm_dict_to_column_features(data, features):
    ...
    Args:
        data (type: pd.DataFrame)  : pd DataFrame
        features (string)          : list of features

    Returns:
        Rtype: pd.DataFrame
        Returns a normalized dataframe with column containing normalized
        dictionary that will span to dataframe columns
    """
    for feature in features:
        normalized_data = data[feature]\
            .map(lambda x: x if isinstance(x, dict)
                 else np.NaN)\
            .apply(pd.Series)\
            .add_prefix('{}.'.format(feature))
        data = pd.concat([data, normalized_data], axis=1).drop(
            [feature, "%s.0" % feature], axis=1)
    return data


def norm_list_dicts_to_rows(data, features):
    """
    Utility function to normalize a column that contains list of dicts to
    separate rows
    ...
    Returns:
        RType: pd.DataFrame
        A normalized dataframe with rows from normalized from list of dicts
    """
    for feature in features:
        data = (
            pd.concat({i: pd.DataFrame(x) for i, x in data[feature].items()})
...
def parallel_func_apply(df, func, no_of_processors, chunksize):
    ...
    Parameter:
         df   (dtype: pd.Dataframe)     : dataframe
         func (dtype: string)           : wrapper function for data processing
         no_of_processors (dtype: int)  : number of processors
...
def get_git_used_script_url(path_to_github_token,
    ...
    try:
        ...
        relative_path = subprocess.check_output(
            ["git", "rev-parse", "--show-prefix"]).decode('ascii').strip()
    except FileNotFoundError:
        raise("Git token not found")
    else:
